  ...
  return async (dispatch) => {
    try {
      const response = await fetch(
        `https://mail-box-client-ade15-default-rtdb.firebaseio.com/${senderEmail}.json`,
        {
          method: 'POST',
      ...
      if (senderEmail !== receiverEmail) {
        await fetch(
          `https://mail-box-client-ade15-default-rtdb.firebaseio.com/${receiverEmail}.json`,
          {
            method: 'POST',
...
export const replaceMail = (emailUrl, loggedUserEmail) => {
  return async (dispatch) => {
    try {
      const response = await fetch(
        `https://mail-box-client-ade15-default-rtdb.firebaseio.com/${emailUrl}.json`
      );
...
export const deleteMail = (mail) => {
  ...
  return async (dispatch) => {
    try {
      const response = await fetch(
        `https://mail-box-client-ade15-default-rtdb.firebaseio.com/${emailUrl}/${mail.id}.json`,
        {
          method: 'DELETE',
        }
      );
